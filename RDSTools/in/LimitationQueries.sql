...
else set @Filetable='Not Supported'
--Stretch DB 
...
Else 
begin 
	set @isreadonly='Not Supported'
	set @role_desc='Not Supported'
end 
-------------------------------
--Online indexes
--SELECT @IsOnlineindex=Case
-- WHEN IIF(count(1)>0,1,0) > 0 then 'Y'
-- ELSE 'N'
-- END 
--FROM (
--SELECT 
...
declare @readoverwrite char(1)
				...
				SELECT    qs.execution_count
					...
					, ( CASE
							WHEN qs.statement_end_offset = -1 THEN LEN( CONVERT( nvarchar(MAX), qt.text )) * 2
								...
								END - qs.statement_start_offset ) / 2 ),(qs.total_logical_reads)*8/1024.0 AS [Total Logical Reads (MB)],
									qs.total_logical_reads as [TotalLogicalRead],qs.total_physical_reads as TotalPhysicalRead,qs.total_logical_writes--, qs.total_grant_kb  
 ...
 ,((TotalLogicalRead*100)/nullif(TotalLogicalRead+total_logical_writes,0)) as readoverwriteweight from Read_WriteIO order by overallreadweight desc
 )
...
@isreadonly as IsReadReplica,@role_desc as DBRole,@IsElasticache as IsElasticache,@IsOnlineindex as isOnlinIndexes ,case when @source is null then 'EC2/onPrem' else @source end as source

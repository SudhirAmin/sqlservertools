...
if (select substring(convert(char(5),SERVERPROPERTY ('productversion')),1,2))>10                        
  begin 
	insert into #SQL_MemCollection
			select x.*,y.*,z.* from (SELECT       getdate() as collectionTime,(bpool_committed/1024) as Commited,(bpool_commit_target/1024)  as targetcommited FROM sys.dm_os_sys_info)  as x,
...
Begin 
	insert into #SQL_MemCollection
		select x.*,y.*,z.* from (SELECT       getdate() as collectionTime,(bpool_committed/1024) as Commited,(SELECT convert(int,[value_in_use])
			FROM sys.configurations WHERE [name] = 'max server memory (MB)')  as targetcommited FROM sys.dm_os_sys_info)  as x,
                           (   SELECT        (total_physical_memory_kb/1024) as totalMem,(available_physical_memory_kb/1024) as AvaiMem FROM sys.dm_os_sys_memory) as y,
                           (SELECT sum(cntr_value)/count(*)  as PLE FROM sys.dm_os_performance_counters WHERE counter_name = 'Page Life expectancy'    AND object_name LIKE '%buffer node%') as Z
End

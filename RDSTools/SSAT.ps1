...
Function ElasticacheAssessment {
  ...
  union
  SELECT 
     sum(convert(bigint,[read])) as [read] 
      ,sum(convert(bigint,[Written])) as write
	  ,(sum(convert(bigint,[read]))+sum(convert(bigint,[written]))) as total
	  ,(sum(convert(bigint,[read]))*100)/(sum(convert(bigint,[read]))+sum(convert(bigint,[written]))) AS ''PercentageofRead''
	  , ''Total Server read%''      
  FROM [dbo].[SQL_DBIO]
  where [read]>0'
  if ($auth -eq 's') {
    ...
    $ElasticAssessoutput = invoke-sqlcmd -serverInstance $dbserver -Database $dbname  -query $ReadOverall 
...
Function DBC {
  $SSATCSV = import-csv C:\RDSTools\out\SQLAssesmentOutput.csv
  $FilePath = "c:\rdstools\out\dbc.csv"
    $FileExists = Test-Path -Path $Filepath
    if ( -not $FileExists) { exit }
    
  $DBCCSV = import-csv C:\RDSTools\out\DBC.csv
if (test-path $sqlserverendpoint) {
    $servercount = (Get-Content $sqlserverendpoint).Length}
  Foreach ($server in $SSATCSV) {
       $serverName = $server.'server Name'
      $serverRDSInstance = $server.'RDS Recommendation based on load'
      $serverRDSInstance = $serverRDSInstance
      $serverRDSConfig = $server.'RDS Recommendation based on current configuration'
      $instance = $serverRDSInstance.split(",")
      $serverRDSInstance = $instance[0]
      $serverRDSInstance = $serverRDSInstance.TrimEnd()
      $EBSType=$server.EBS
      $IOPS=$server.'MAX_Totaliops'
      $throughput=$server.'MAX_ThroughPut(MB)'
      $CPUOptimized=$server.'CPU Pressure Utilization(%)'
<#      $row = 2
...
Foreach ($DBCserver in $DBCCSV) 
{
if ($DBCserver.servername -eq $serverName)
{$DBCserver.InstanceType=$serverRDSInstance
 $DBCserver.EBSType=$EBSType
$DBCserver.IOPS=$IOPS
$DBCserver.Throughput=$throughput
$DBCserver.CpuUtilization=$CPUOptimized
}
...
$DBCCSV | export-csv C:\RDSTools\out\DBC.csv -NoTypeInformation
}
Function TCO {
  try {
    ...
    Foreach ($server in $tcocsv) {
      ...
      for ($i = 1; $i -le $servercount; $i++) {
        ...
        if ($servername -eq $server.'Server Name') {
          $ExcelWorkSheet.Cells.Item($row, 11) =  $serverRDSConfig
...
"@
  ...
  @{Name = " MAX_Totaliops "; Expression = { $_.'MAX_Totaliops' } }, @{Name = "Throughput"; Expression = { $_.'MAX_ThroughPut(MB)' } }, @{Name = " SQl server edition "; Expression = { $_.'SQl server edition' } }, @{Name = " SQl server Version "; Expression = { $_.'Sql server version' } },
...
function DB_level {
 ...
 $DBLEVELsql = '
  ...
  $targetfile = "c:\rdstools\out\" + ($dbserver.replace('\', '~').Toupper()) + "_" + $dbtypeExt + "_" + $timestamp + "_dblevel.csv"
...
function RDSInstance {
  ...
  elseif ($ratio -gt 8 -and $ratio -le 15.25) {
    $ratio = 15
    $type = 'M'
  }
  elseif ($ratio -gt 15 -and $ratio -le 30) {
          $ratio = 30
          $type = 'M'
     }
  elseif ($ratio -gt 30 ) {
  ...
  elseif ($Custom -eq 'N' -and $ratio -ge 15) {
    $file = $fileoriginal | where-Object { [int]$_.ratio -eq [int]$ratio -and $_.size -eq $class -and [int]$_.iops -ge [int]$Totaliops -and [int]$_.Throughput -ge [int]$Throughput -and $_.instancetype -eq $type -and $_.edition -eq $edition -and $_.version -match $version }
...
"
  $ThroughputIOPS = "With AVGIO (TotalIOPs,Throughput)
			        ...
			        select  Maxiops.*,Miniops.*,AVGIOP.*
                            from (SELECT top 1 isnull(sum(TotalIOPs)/60,0) as Max_IOPS,isnull(((sum(BRead+BWritten)/60)/1048576),0) as Max_Throughput
                                    FROM [msdb].[dbo].[SQL_DBIO]
                                      where TotalIOPs>0
			                        group by Sample_ID
			                        order by Max_IOPS desc ) as Maxiops,
                                (SELECT top 1 isnull(sum(TotalIOPs)/60,0) as Min_iops,isnull(((sum(BRead+BWritten)/60)/1048576),0) as Min_Throughput
                                    FROM [msdb].[dbo].[SQL_DBIO]
                                      where TotalIOPs>0
			                        group by Sample_ID
  ...
  $sqlVE = 'SELECT   case  WHEN CONVERT(VARCHAR(128),SERVERPROPERTY(''Edition'')) like ''Standard%'' Then ''SE''
        ...
        CASE
            ...
            WHEN CONVERT(VARCHAR(128), SERVERPROPERTY (''productversion'')) like ''16%'' THEN ''16''
               Else ''12''
        end AS ProductVersion'
   $sqldbcount='select count(*) as DBCount  from sys.databases where database_id>4'
   $dbsize='SELECT isnull(round(sum((cast(size as bigint)*8))/1024/1024,2),0) as TotalDBSize FROM master.sys.master_files where database_id>4'
  if ($auth -eq 'W') {
    ...
    $dbcount = invoke-sqlcmd -serverInstance $dbserver -Database $dbname  -query $sqldbcount
    $dbSize=invoke-sqlcmd -serverInstance $dbserver -Database $dbname  -query $dbsize
  }
  else {
    ...
    $dbcount = invoke-sqlcmd -serverInstance $dbserver -Database $dbname  -query $sqldbcount -user $User -password $Savepass
    $dbSize=invoke-sqlcmd -serverInstance $dbserver -Database $dbname  -query $dbsize -user $User -password $Savepass
  }
  $classtemp = ''
  ...
  $dbsize=$dbsize.TotalDBSize
  $dbcount=[int]$dbcount.dbcount
  if ($TotalIOPS -ge '16000' )
   { $EBS ='IO1'}
   else {$EBS ='GP3'}
  ...
  if ($elasticache -eq 'Y') {
    ...
    $val = [pscustomobject]@{'Server Name' = $dbserver; 'Logical CPU Count' = $cpuonPrem; 'MaxMemorySettings GB' = $RamonPrem; 'Collection Time' = $finalcollectionTime ;
       'CPU Pressure Utilization(%)' = $CPURecoResult.utilization; 'CPu95Percentile' = $Cpupercentile; 'Total CPU Utilization(%)' = $cpuTUtilization; 
      'Server Memory Utlization%' = $MemRecoResult.utilization; 'MAX_Totaliops' = $Totaliops; 'MAX_ThroughPut(MB)' = $throughput; 'MIN_Totaliops' = $ThroughputIOPS.MINIOPS; 'MIN_ThroughPut(MB)' = $ThroughputIOPS.MIN_Throughput; 'AVG_Totaliops' = $ThroughputIOPS.AVG_IOPS; 'AVG_ThroughPut(MB)' = $ThroughputIOPS.AVG_Throughput; 'SQl server edition' = $SQLVEresult.edition; 'Sql server version' = $SQLVEresult.productversion;
      'Elasticache' = $Elasticoutput.elasticache;'DBCount'=$dbcount;'DBTtoalSize(GB)'=$dbsize;'Mem Recommendation' = $MemRecoResult.MemRecomme;'CPU Recommendation' = $CPURecoResult.CpuRecomme;
      'RDS Recommendation based on current configuration' = $Classonprem; 'RDS Recommendation based on load' = $rdsinstance; 'EBS'=$EBS;'Note' = 'Check the detailed Elasticache report for the Read over write per DB in rdstools\out'
    }
  }
    ...
    if ($options -contains '95') {
      $val = [pscustomobject]@{'Server Name' = $dbserver; 'Logical CPU Count' = $cpuonPrem; 'MaxMemorySettings GB' = $RamonPrem; 'Collection Time' = $finalcollectionTime;
        'CPU Pressure Utilization(%)' = $CPURecoResult.utilization; 'CPu95Percentile' = $Cpupercentile; 'Total CPU Utilization(%)' = $cpuTUtilization;
        'Server Memory Utlization%' = $MemRecoResult.utilization; 'MAX_Totaliops' = $Totaliops; 'MAX_ThroughPut(MB)' = $throughput; 'MIN_Totaliops' = $ThroughputIOPS.MINIOPS; 'MIN_ThroughPut(MB)' = $ThroughputIOPS.MIN_Throughput; 'AVG_Totaliops' = $ThroughputIOPS.AVG_IOPS; 'AVG_ThroughPut(MB)' = $ThroughputIOPS.AVG_Throughput; 'SQl server edition' = $SQLVEresult.edition; 'Sql server version' = $SQLVEresult.productversion
        ;'DBCount'=$dbcount;'DBTtoalSize(GB)'=$dbsize; 'CPU Recommendation' = $CPURecoResult.CpuRecomme; 'Mem Recommendation' = $MemRecoResult.MemRecomme;'RDS Recommendation based on current configuration' = $Classonprem; 'RDS Recommendation based on load' = $rdsinstance;'EBS'=$EBS; 'Note' = $Note
      }
    }#if
    else {
      $val = [pscustomobject]@{'Server Name' = $dbserver; 'Logical CPU Count' = $cpuonPrem; 'MaxMemorySettings GB' = $RamonPrem; 'Collection Time' = $finalcollectionTime ;
         'CPU Pressure Utilization(%)' = $CPURecoResult.utilization; 'CPu95Percentile' = $cpupercentile; 'Total CPU Utilization(%)' = $cpuTUtilization;
        'Server Memory Utlization%' = $MemRecoResult.utilization; 'MAX_Totaliops' = $Totaliops; 'MAX_ThroughPut(MB)' = $throughput; 'MIN_Totaliops' = $ThroughputIOPS.MIN_IOPS; 'MIN_ThroughPut(MB)' = $ThroughputIOPS.MIN_Throughput; 'AVG_Totaliops' = $ThroughputIOPS.AVG_IOPS; 'AVG_ThroughPut(MB)' = $ThroughputIOPS.AVG_Throughput; 'SQl server edition' = $SQLVEresult.edition; 'Sql server version' = $SQLVEresult.productversion;
        'DBCount'=$dbcount;'DBTtoalSize(GB)'=$dbsize; 'Mem Recommendation' = $MemRecoResult.MemRecomme;'CPU Recommendation' = $CPURecoResult.CpuRecomme;'RDS Recommendation based on current configuration' = $Classonprem; 'RDS Recommendation based on load' = $rdsinstance; 'RDS Recommendation based on 95 percentile' = $class95; 'EBS'=$EBS;'Note' = $Note
      }
    }#else
  }#top else
...
function Get-SQLStatus {
  ...
  return $action, [int]$SQLStatus.TimeRemaining,[int]$sqlStatus.Max_Sample_ID
}#Function status
...
#--------------
# Validate parameters
if (-not $auth) { throw "Authentication parameter is required." }
if (-not $login -and $auth -eq 'S') { throw "Login parameter is required." }
if (-not $password -and $auth -eq 'S') { throw "Password parameter is required." }
if ($collectiontime -le 0) { throw "Collection time must be a positive integer." }
if (-not (Test-Path $sqlserverendpoint )-and $options -ne 'upload') { throw "SQL Server endpoint path does not exist." }
if ($Elasticache -notin @('Y', 'N')) { throw "Elasticache must be 'Y' or 'N'." }
if ($TCOOnly -notin @('Y', 'N')) { throw "TCOOnly must be 'Y' or 'N'." }

# Your script logic here
try {
    # Example: Connect to a database
    # Connect-Database -Auth $auth -Login $login -Password $password
